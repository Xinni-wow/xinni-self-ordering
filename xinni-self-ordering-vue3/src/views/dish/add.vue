...
const addFlavorValue = (index: number) => {
  const value = newFlavorVal.value[index]?.trim()
  if (!value) return
  if (!form.dishFlavors[index].list.includes(value)) {
    form.dishFlavors[index].list.push(value)
  }
  newFlavorVal.value[index] = ''
}
              ...
              <div v-for="(item, index) in form.dishFlavors" :key="index" class="items">
                ...
                <div class="labItems" style="display: flex">
                  <span v-for="(it, ind) in item.list" :key="ind">{{ it }}
                    ...
                    <el-input
  v-model="newFlavorVal[index]"
  placeholder="输入值后回车添加"
  size="small"
  @keyup.enter.native="addFlavorValue(index)"
